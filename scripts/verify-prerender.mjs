...
for (const [market, config] of Object.entries(MARKETS)) {
  ...
  for (const entry of entries) {
    ...
    expect(!new RegExp(`(?:src|href)="https://${config.host}/assets/`, 'i').test(html), `${label} hardcodes a regular asset origin`)
    ...
    expect(new RegExp(`<meta property="og:image" content="https://${config.host}/[^\"]+"`, 'i').test(html), `${label} lacks an absolute market OG image`)
    expect(new RegExp(`<meta name="twitter:image" content="https://${config.host}/[^\"]+"`, 'i').test(html), `${label} lacks an absolute market Twitter image`)
