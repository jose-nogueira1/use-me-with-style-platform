...
| --- | --- | --- |
| `usemewithstyle.com` | Vercel storefront | Geo-detect: Angola to `ao`, all other countries to `pt` |
| `www.usemewithstyle.com` | Vercel storefront | Same as apex; may be redirected to apex by Vercel |
| `ao.usemewithstyle.com` | Vercel storefront | Angola catalogue, Kz pricing, AO stock and checkout |
| `pt.usemewithstyle.com` | Vercel storefront | Portugal catalogue, EUR pricing, PT stock and checkout |
| `cms.usemewithstyle.com` | Railway CMS | Payload Admin and API |

## Cutover checklist

1. Purchase and confirm ownership of the final apex domain. If it differs from `usemewithstyle.com`, update `.env.production` and every hostname in this document first.
...
5. Set the CMS `CORS_ORIGINS` to the Vercel canonical URL plus the four storefront origins. Set `PUBLIC_SITE_URL=https://usemewithstyle.com`. The apex is intentional: Stripe returns there and the storefront routes to a market.
6. Set storefront `VITE_SITE_URL=https://usemewithstyle.com` and `VITE_CMS_URL=https://cms.usemewithstyle.com/admin`; redeploy production. The browser API remains same-origin through the Vercel `/api/*` rewrite.
